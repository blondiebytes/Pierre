...
class MailboxViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        scrollView.frame = CGRect(x: self.view.frame.width, y: self.view.frame.height, width: self.view.frame.width/4, height: self.view.frame.height/2)
        
        
        scrollView.contentSize = CGSize(width: scrollView.bounds.size.width/4 , height: 300)
        ...
        for row in 0...(numRows - 1) {
            for column in 0...(numColumns - 1) {
                ...
                var yPos:CGFloat = CGFloat(row * 2) * currentLetter.bounds.size.height
                ...
                if (column == 0) {
                    ...
                    var xPos:CGFloat = self.scrollView.bounds.size.width - currentLetter.bounds.size.width/2
                    currentLetter.frame = CGRect(x:xPos, y: yPos, width: letterWidth, height: letterHeight)
                    
                    println("this is the midx: ", currentLetter.frame.midX)
                    ...
                    var xPos: CGFloat = 2 * (self.scrollView.bounds.size.width) - letterWidth
                    currentLetter.frame = CGRect(x: xPos , y:yPos, width: letterWidth, height:letterHeight)
                }
    ...
    func handleTap (sender:UITapGestureRecognizer) {
        ...
        if (singleLetterView.shouldShow!) {
            ...
            let secondCol:Bool = location.x > (self.scrollView.bounds.size.width/2 - dummyLetter.bounds.size.width/2) && location.x < (self.scrollView.bounds.size.width/2 + dummyLetter.bounds.size.width/2)
            let thirdCol:Bool = location.x > (self.scrollView.bounds.size.width - dummyLetter.bounds.size.width) && location.x < (self.scrollView.bounds.size.width)
            
            
            //booleans if within the rows
            let firstRow:Bool = location.y > 0 && location.y < letterHeight
            let secondRow:Bool = location.y > 2 * dummyLetter.bounds.size.height && location.y < 3 * dummyLetter.bounds.size.height
            let thirdRow:Bool = location.y > 4 * dummyLetter.bounds.size.height && location.y < 5 * dummyLetter.bounds.size.height
